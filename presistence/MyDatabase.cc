...
DCMysql::DCMysql(const string& sHost, const string& sUser, const string& sPasswd, const string& sDatabase, const string &sCharSet, int port, int iFlag)
:_bConnected(false)
{
...
DCMysql::DCMysql(const DCDBConf& tcDBConf)
:_bConnected(false)
{
    _dbConf = tcDBConf;
...
void DCMysql::init(const string& sHost, const string& sUser, const string& sPasswd, const string& sDatabase, const string &sCharSet, int port, int iFlag)
{
    _dbConf._host = sHost;
    _dbConf._user = sUser;
...
void DCMysql::init(const DCDBConf& tcDBConf)
{
    _dbConf = tcDBConf;
}

void DCMysql::connect()
{
    disconnect();
    ...
    if(!_dbConf._charset.empty())  {
        if (mysql_options(_pstMql, MYSQL_SET_CHARSET_NAME, _dbConf._charset.c_str())) {
    		throw DCMysql_Exception(string("DCMysql::connect: mysql_options MYSQL_SET_CHARSET_NAME ") + _dbConf._charset + ":" + string(mysql_error(_pstMql)), mysql_errno(_pstMql));
        }
    }
    ...
    {
        throw DCMysql_Exception("[DCMysql::connect]: mysql_real_connect: " + string(mysql_error(_pstMql)), mysql_errno(_pstMql));
    }
    
	_bConnected = true;
}

void DCMysql::disconnect()
{
    if (_pstMql != NULL)
    {
        mysql_close(_pstMql);
...
string DCMysql::escapeString(const string& sFrom)
{
	if(!_bConnected)
	{
		connect();
...
MYSQL *DCMysql::getMysql(void)
{
    return _pstMql;
}

string DCMysql::buildInsertSQL(const string &sTableName, const RECORD_DATA &mpColumns)
{
...
string DCMysql::buildReplaceSQL(const string &sTableName, const RECORD_DATA &mpColumns)
{
    ostringstream sColumnNames;
    ostringstream sColumnValues;
...
string DCMysql::buildUpdateSQL(const string &sTableName,const RECORD_DATA &mpColumns, const string &sWhereFilter)
{
...
string DCMysql::getVariables(const string &sName)
{
...
void DCMysql::execute(const string& sSql)
{
	/**
	没有连上, 连接数据库
	*/
    ...
    if (iRet != 0)
    {
        throw DCMysql_Exception("[DCMysql::execute]: mysql_query: [ " + sSql+" ] :" + string(mysql_error(_pstMql)), mysql_errno(_pstMql));
    }
}

DCMysql::MysqlData DCMysql::queryRecord(const string& sSql)
{
    MysqlData   data;
	...
	*/
    ...
    if(pstRes == NULL)
    {
        throw DCMysql_Exception("[DCMysql::queryRecord]: mysql_store_result: " + sSql + " : " + string(mysql_error(_pstMql)), mysql_errno(_pstMql));
    }
...
size_t DCMysql::updateRecord(const string &sTableName, const RECORD_DATA &mpColumns, const string &sCondition)
{
...
size_t DCMysql::insertRecord(const string &sTableName, const RECORD_DATA &mpColumns)
{
...
size_t DCMysql::replaceRecord(const string &sTableName, const RECORD_DATA &mpColumns)
{
...
size_t DCMysql::deleteRecord(const string &sTableName, const string &sCondition)
{
    ostringstream sSql;
...
size_t DCMysql::getRecordCount(const string& sTableName, const string &sCondition)
{
    ostringstream sSql;
...
size_t DCMysql::getSqlCount(const string &sCondition)
{
    ostringstream sSql;
...
int DCMysql::getMaxValue(const string& sTableName, const string& sFieldName,const string &sCondition)
{
    ostringstream sSql;
...
bool DCMysql::existRecord(const string& sql)
{
    return queryRecord(sql).size() > 0;
}

long DCMysql::lastInsertID()
{
    return mysql_insert_id(_pstMql);
}

size_t DCMysql::getAffectedRows()
{
	return mysql_affected_rows(_pstMql);
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////
DCMysql::MysqlRecord::MysqlRecord(const map<string, string> &record)
: _record(record)
{
}

const string& DCMysql::MysqlRecord::operator[](const string &s) const
{
    map<string, string>::const_iterator it = _record.find(s);
    if(it == _record.end())
    {
        throw DCMysql_Exception("field '" + s + "' not exists.", -1);
    }
    return it->second;
}
...
DCMysql::MysqlData::MysqlData(const DCMysql::MysqlData& r)
{
    this->_data = r._data;
    _errno = r._errno;
    _last_insert_id = r._last_insert_id;
    _affect_rows = r._affect_rows;
    _err_str = r._err_str;
}


vector<map<string, string> >& DCMysql::MysqlData::data()
{
    return _data;
}

size_t DCMysql::MysqlData::size()
{
    return _data.size();
}

DCMysql::MysqlRecord DCMysql::MysqlData::operator[](size_t i)
{
